...
def _save_plot(rows, output_path, num_requests, trials, batch_sizes):
    ...
    axes[0].bar(batches, throughput, color="tab:blue", yerr=throughput_std, capsize=3)
    ...
    axes[1].errorbar(batches, avg, yerr=avg_std, marker="o", label="Avg", capsize=3)
    ...
    bar_width = 0.35
    offsets = [b - (bar_width / 2.0) for b in batches]
    offsets2 = [b + (bar_width / 2.0) for b in batches]
    axes[2].bar(
        offsets,
        p50,
        width=bar_width,
